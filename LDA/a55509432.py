...
# file path
tranfile = os.path.join(path, os.path.normpath(conf.get("filepath","trainfile")))
...
# iter_times = 100
# top_words_num = 20
K = int(conf.get("model_args","alpha"))
...
class LDAModel(object):
    ...
    def __init__(self, dpre):
        ...
        self.dpre = dpre # get data preprocessing
        ...
        # [[lendoc1][lendoc2][lendoc3][lendoc4]...]

        # 随机先分配类型
        for x in xrange(len(self.Z)):
            self.ndsum[x] = self.dpre.docs[x].length
            for y in xrange(self.dpre.docs[x].length): # for each doc
    ...
    def sampling(self, i, j):
        ...
        Vbeta = self.dpre.word_counts * self.beta
    ...
    def est(self):
        ...
        for x in xrange(self.iter_times):                   # for each iter
            for i in xrange(self.dpre.docs_count):          # for each doc
                for j in xrange(self.dpre.docs[i].length):  # for each words
                    topic = self.sampling(i, j)             # return topic
                    self.Z[i][j] = topic                    # write to Z[][]
        logger.info(u"迭代完成。")
